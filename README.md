...
| `CLOUD_B02_MAX` | 云过滤阈值（B02 反射率），默认 0.25，调小=更严格 |
| `MIN_AREA_M2` | 最小水体面积（m²），过滤噪声小斑块 |
| `CLOSING_M` | 形态学闭运算填补间隙距离（米），0=不做；30m 可修复河流因误差断裂的问题 |
...
| S2B_MSIL1C_20260202 | 2026-02-02 | 4 | NDWI | 100000 m²（0.1 km²） | 2201 |

## 参数调优记录（S2B_20260202）

对照 oracle 数据（1251 个要素，1441 km²）测试不同阈值组合，最终采用：

| 参数 | 值 | 说明 |
|---|---|---|
| `NDWI_THRESH` | -0.1 | 默认 0.0 偏保守，调低后总面积更接近 oracle |
| `CLOUD_B02_MAX` | 0.25 | 云过滤对本景影响极小 |
| `MIN_AREA_M2` | 100000 | 与 oracle 一致 |

加入形态学闭运算（30m）后：2201 个要素，4097 km²。闭运算将河流中间因像元误差断裂的片段重新连通，使原本面积不足被过滤的河段得以保留。  
剩余与 oracle 的差异来自无 SWIR 波段（无法使用 MNDWI）以及矢量化方式差异。
